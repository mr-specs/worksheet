...
//login validation
    ...
    public @ResponseBody ResponseEntity<List<User>> getUserDetails(@RequestBody User currentUser) {
        System.out.println("uname+pwd --------->"+currentUser.name+currentUser.password);
        List<User> loginUser = userService.getUsers().stream().filter(x->x.name.equals(currentUser.name) && x.password.equals(currentUser.password)).collect(Collectors.toList());
    ...
    public @ResponseBody ResponseEntity<List<User>> getAllUsers() {
        List<User> userOnly = userService.getUsers().stream().filter(x->x.role.equals("U")).collect(Collectors.toList());
